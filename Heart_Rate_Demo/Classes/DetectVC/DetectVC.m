...
- (void)viewDidLoad {
    ...
    ResultVCObj = [[ResultVC alloc] initWithNibName:@"ResultVC" bundle:nil];
...
-(void) viewWillAppear:(BOOL)animated {
    ...
    intSecond = 0;
    [timerUpdatePulse invalidate];
    timerUpdatePulse = nil;
    lblRemainingTime.text = [NSString stringWithFormat:@"Valid Frames: %d%%", MIN(100, (100 * validFrameCounter)/MIN_FRAMES_FOR_FILTER_TO_SETTLE)];
...
#pragma mark -
#pragma mark start capturing frames
-(void) startCameraCapture {
    ...
    timerUpdatePulse = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(update) userInfo:nil repeats:YES];
}
...
#pragma mark -
#pragma mark process the frame of video
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if(s>0.5 && v>0.5) {
        ...
        if(validFrameCounter > MIN_FRAMES_FOR_FILTER_TO_SETTLE) {
            ...
            [self.simpleChart performSelectorOnMainThread:@selector(addPoint:) withObject:[NSNumber numberWithFloat:filtered] waitUntilDone:NO];
...
-(void) update {
    ...
    if(intSecond >= 10) {
        ...
        lblBPMValue.text=[NSString stringWithFormat:@"%0.0f", pulse];
        
        [timerUpdatePulse invalidate];
        timerUpdatePulse = nil;
        
        NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithDictionary:[[Singleton sharedSingleton] getDictUserData]];
        [dict setValue:[NSString stringWithFormat:@"%0.0f", pulse] forKey:g_Key_BPM];
        [[Singleton sharedSingleton] setDictUserData:dict];
        [self.navigationController pushViewController:ResultVCObj animated:true];
        ...
        if(avePeriod==INVALID_PULSE_PERIOD) {
            ...
            if(MIN(100, (100 * validFrameCounter)/MIN_FRAMES_FOR_FILTER_TO_SETTLE) >= 100) {
                lblRemainingTime.text = @"Detecting Pulse...";
            } else {
                lblRemainingTime.text = [NSString stringWithFormat:@"Valid Frames: %d%%", MIN(100, (100 * validFrameCounter)/MIN_FRAMES_FOR_FILTER_TO_SETTLE)];
            }
        } else {
            
            lblRemainingTime.text = [NSString stringWithFormat:@"%ds Left",10 - intSecond];
